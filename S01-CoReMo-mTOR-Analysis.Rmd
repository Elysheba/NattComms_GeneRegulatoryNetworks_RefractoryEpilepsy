...
subtitle: "Coregulated gene modules identification in mTORopathies (TSC, FCD 2a, FCD 2b)"
...
## Mutations
pcMut <- p.adjust(apply(
   pca$x, 2,
   function(pc){
     head(pc)
      toTest <- data.frame(
         val=pc,
         groups=as.factor(sampleInfo$Gene[match(rownames(pca$x),sampleInfo$GS_RNASEQ_ID)])
      )
      summary(aov(formula=val~groups, data=toTest))[[1]]$`Pr(>F)`[1]
   }
), method="BH")
...
ageAss <- p.adjust(apply(
   ...
   function(pc){
      toTest <- data.frame(
         val=pc,
         groups=sampleInfo$Age_At_Time_of_Operation[match(rownames(pca$x),sampleInfo$GS_RNASEQ_ID)])
      summary(aov(formula=val~groups, data=toTest))[[1]]$`Pr(>F)`[1]
   }
), method="BH")
##
durAss <- p.adjust(apply(
   ...
   function(pc){
      toTest <- data.frame(
         val=pc,
         groups=sampleInfo[match(rownames(pca$x),sampleInfo$GS_RNASEQ_ID), c("Age_At_Time_of_Operation", "Duration_years")])
...
toShow<-data.frame(cbind(pcLib,pcsgid, pcDiag,pcArea, ageAss, durAss, pcMut))
```
...
p <- ggplot(toPlot) +
  geom_point(aes(x = PC1, y = PC2, shape = PA_Diagnosis, 
                 col = Levetiracetam), 
             size = 2) +
  theme_bw() 
ggplotly(p)
...
```{r, include = TRUE, fig.height=5}
toPlot <- sampleInfo %>% 
  bind_cols(as_tibble(pca$x[sampleInfo$GS_RNASEQ_ID,]))

p <- ggplot(toPlot) +
  geom_point(aes(x = PC1, y = PC3, col = PA_Diagnosis, shape = Library_Prep_batch), size = 2) +
  theme_bw() 
ggplotly(p)
```


## FCD 2a, FCD 2b, TSC, and control samples
...
sampleInfo <- sampleInfo %>% 
    filter(PA_Diagnosis %in% c("Control", "FCD 2a", "FCD 2b", "TSC") &  
             !Area_of_Resection %in% c("Hippocampus")) %>%
  filter(!GS_RNASEQ_ID %in% c("103866-001-192")) %>% ## Only sample from temporal lobe in library prep batch E
  mutate(log10_seizure_freq = log10(Seizure_Frequency_months)) %>% 
...
```{r, include= TRUE}
ggplot(sampleInfo, aes(x = PA_Diagnosis, y = Age_At_Time_of_Operation)) +
  geom_boxplot()
```

```{r}
if(file.exists(here("Data/04_QC/mTOR_expression_matrix_batch_corrected.rda"))){
  ...
  bc_vg <- removeBatchEffect(d,
                             ...
                             design = design)
  save("bc_vg", 
       file = here("Data/04_QC/mTOR_expression_matrix_batch_corrected.rda"))
}
```
...
durAss <- p.adjust(apply(
   ...
   function(pc){
      toTest <- data.frame(
         val=pc,
         groups=sampleInfo[match(rownames(pca$x),sampleInfo$GS_RNASEQ_ID),
                           c("Age_At_Time_of_Operation", "Duration_years")])
       drop1(lm(data=toTest,
                 formula=val~groups.Duration_years+groups.Age_At_Time_of_Operation),
             test="F")[2,"Pr(>F)"]
   }
...
```{r, include = TRUE, fig.height=5}
toPlot <- sampleInfo %>% 
  bind_cols(as_tibble(pca$x[sampleInfo$GS_RNASEQ_ID,1:40]))

p <- ggplot(toPlot) +
  geom_point(aes(x = PC1, y = PC2, 
                 col = PA_Diagnosis, 
                 shape = Area_of_Resection), 
             size = 2) +
  theme_bw() 
ggplotly(p)
```
...
This reports deals with the identification of modules of gene coregulation in mTORopthies and assessment of the conservation of the identified regulations in other contexts. As explored in the [previous report](S03-Explore-Study-Design), the coregulation modules will be build within each condition with sufficient amount of samples and within each tissue to avoid interference of tissue-specific regulations. For mTORopathies (mTOR) there are `r sampleInfo %>% filter(PA_Diagnosis %in% c("FCD 2a", "FCD 2b", "TSC")) %>% nrow()` samples that will be used to identified gene modules of coregulation. Library preparation batch was identified to impact gene expression for mTORopathies. Therefore, a batch correction was performed using *limma* R package to avoid that library preparation batch might drive the module identification.
...
<li> Gene filtering based on variance in expression in mTORopathies </li>
...
Due to the unbalance between mTORopathies samples (`r all_samples %>% filter(PA_Diagnosis %in% c("FCD 2a", "FCD 2b", "TSC")) %>% nrow()` samples) and control samples (`r all_samples %>% filter(!PA_Diagnosis %in% c("FCD 2a", "FCD 2b", "TSC")) %>% nrow()` samples), there may be more genes significantly differentially expressed between the two groups ([Yang et al. 2006](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC1780111/)).


```{r deg, cache=FALSE,echo=FALSE}
resFile <- here("Data/06_Modules/mTOR/Module-Data","DEG.rda")

if(file.exists(resFile)){
 load(resFile)
}else{
  s <- all_samples %>% 
    mutate(PA_Diagnosis = case_when(PA_Diagnosis != "Control" ~ "mTOR",
                                    TRUE ~ PA_Diagnosis))
  dd <- vg$E[,pull(s, GS_RNASEQ_ID)]
  ...
  fit <- lmFit(object = dd,
               design = design)
  ##
  contrast.matrix <- makeContrasts(mTOR-Control, levels = design)
...
```{r mad,echo=FALSE}
sampleInfo <- sampleInfo %>% filter(PA_Diagnosis %in% c("FCD 2a", "FCD 2b", "TSC"))
...
length(KeptProbes)
save(KeptProbes, 
     file = here("Data","06_Modules/mTOR/Module-Data", "List-KeptProbes.rda"))
```
...
## Create Spearman correlation matrix for all genes displaying variance (MAD > 0.25) 
corMeth <- "spearman"
sampGroup <- "mTOR"
# sampleInfo <- sampleInfo %>% filter(PA_Diagnosis %in% c("FCD 2a", "FCD 2b", "TSC"))
d <- bc_vg[KeptProbes,pull(sampleInfo,GS_RNASEQ_ID)]
dim(d)
corFile <- here("Data","06_Modules/mTOR/Module-Data",
                paste0(corMeth,"-KeptProbes-",sampGroup,".rda"))
##
if(!file.exists(here("Data","06_Modules","mTOR/Module-Data/Modules-mTOR.rda"))){
...
r2cut <- 0.85
sFile <- here("Data", "06_Modules", "mTOR/Module-Data/ScaleFreeIndex_mTOR.rda")
...
corFile <- here("Data", "06_Modules", "mTOR", "ClustModules", "Clustering_mTOR.rda")
##
if(file.exists(corFile)){
  load(corFile)
}else{
...
load(here("Data/06_Modules/mTOR/Module-Data/Bootstrapping_Similarity_mTOR.rda"))
```
...
if(file.exists(here("Data/06_Modules/mTOR/Module-Data/mTOR_CorrMembership.rda"))){
  load(here("Data/06_Modules/mTOR/Module-Data/mTOR_CorrMembership.rda"))
}else{
  ...
  save(data.clusters, data.cormat, data.modcor, 
       file = here("Data/06_Modules/mTOR/Module-Data/mTOR_CorrMembership.rda"))
}
```
...
Data of the modules is available in the **Data/06_Modules/HS** folder.
...
corFile <-  here("Data","06_Modules","mTOR/Module-Data/Modules-mTOR.rda")
if(file.exists(corFile)){
  load(corFile)
}else{
  modules <- list()
  ...
  for(mid in unique(data.clusters)){
    ...
    resFile2 <- here("Data/06_Modules/mTOR/ClustModules", paste0(mid, ".rda"))
...
diagAss <- p.adjust(apply(
   egg[,grep("mTOR[.]",colnames(egg))], 2,
   function(pc){
      toTest <- data.frame(
         val=pc,
         groups=as.factor(egg$PA_Diagnosis)
      )
      summary(aov(formula=val~groups, data=toTest))[[1]]$`Pr(>F)`[1]
   }
), method="BH")
...
modulesTable <- tibble(Module = names(modules.Ensembl), 
                       ...
                       `O Size` = o.size,
                       `U Size` = u.size,
                       `Diagnosis` = diagAss[names(modules.Ensembl)],
...
save(modulesTable, 
     file = here("Data", "06_Modules/mTOR/Module-Data/PhenotypeAssociation.rda"))
```

```{r, include = TRUE}
shown <- datatable(modulesTable) %>%
  formatSignif(c(3,6:12), digits = 2) %>%
  formatStyle(
     c(6:12),
...
if(file.exists( here("Data/06_Modules/mTOR/Conservation/mTOR_ConservationResults.rda"))){
  load( here("Data/06_Modules/mTOR/Conservation/mTOR_ConservationResults.rda"))
}else{
  ## Reference = mTOR modules ----
  load(here("Data/06_Modules/mTOR/Conservation/mTOR_Conservation.rda"))
  ...
  crosstabulation <- do.call(rbind,
                          lapply(names(toRet),
                                 function(x){
                                   toRet <- toRet[[x]]$crossTab %>%
                                     # filter(FDR <= 0.05) %>%
                                     mutate(Project = sub("_.*", "", x),
                                            Condition = sub(paste0(unique(Project),"_"), "", x)) %>% 
                                     select(Project,
                                            Condition,
                                            Module = q,
                                            `Module size` = qsize,
    ...
    ## AMC controls
  load(here("Data/06_Modules/mTOR/Conservation/mTOR_AMC_Control_Conservation.rda"))
  # names(toRet) <- paste("GTEx", gsub("Expression in Brain - ", "", names(toRet)), sep = "_")
  toAdd <- do.call(rbind,
                          lapply(names(toRet),
                                 function(x){
                                    toRet <- toRet[[x]]$conservation %>%
  ...
  save(conservation, crosstabulation, 
       file = here("Data/06_Modules/mTOR/Conservation/mTOR_ConservationResults.rda"))
}
```
...
Using a hypergeometric test, the overlap of gene content between different modules across studies can be assessed. The table below shows the modules significantly overlapping the mTOR modules. 
...
To establish which modules are differentially co-expressed between mTOR and control samples we compare the difference in median R² with the null distribution created by permuting over samples (number of permutations: 10,000).


```{r,echo=FALSE,include=TRUE,eval = TRUE}
if(file.exists(here("Data/06_Modules/mTOR/Module-Data/DifferentialCoexpression_mTOR.rda"))){
  load(here("Data/06_Modules/mTOR/Module-Data/DifferentialCoexpression_mTOR.rda"))
}else{
 source("S13-Differential-Coexpression-mTOR.R")
}
```

<br>
The table below shows the median R² values for mTOR and control samples, difference in median R² between the two conditions, and the empirical p-value. 
<br>

```{r}
toShow <- bind_cols(diffco[[4]] %>% 
                      select(Module, R2_Control, 
                             R2_mTOR = R2_Disease, pvalue_mTOR_Control = pvalue) %>% 
                      arrange(Module),
                    diffco[[1]] %>% 
                      arrange(Module) %>% 
                      select(R2_FCD2a, R2_FCD2b, pvalue_FCD2a_FCD2b = pvalue),
                    diffco[[2]] %>% 
                      arrange(Module) %>% 
                      select(R2_TSC, pvalue_FCD2a_TSC = pvalue),
                    diffco[[3]] %>% 
                      arrange(Module) %>% 
                      select(pvalue_FCD2b_TSC = pvalue)) 
toShow <- toShow %>% 
  select(Module, R2_Control, R2_FCD2a, R2_FCD2b, R2_TSC, R2_mTOR, pvalue_FCD2a_FCD2b,
         pvalue_FCD2a_TSC, pvalue_FCD2b_TSC, pvalue_mTOR_Control)
```
...
```{r}
toPlot <- toShow %>% select(Module, R2_Control, R2_FCD2a,
                            R2_FCD2b, R2_TSC) %>% 
  tidyr::gather(key = cohort, 
                value = "R2", R2_Control, R2_FCD2a,
                R2_FCD2b, R2_TSC) %>% 
  mutate(cohort = factor(gsub("R2_", "", cohort), 
                         levels = c("Control", "FCD2a", "FCD2b", "TSC")))

p <- ggplot(toPlot) +
  geom_point(aes(x = cohort, y = R2, color = Module)) +
  geom_line(aes(x = cohort, y = R2, color = Module, group = Module)) +
  theme_classic()
ggplotly(p)
```
...
if(file.exists(here("Data/07_CausalReasoning/mTOR_regulators_lp.rda"))){
  load(here("Data/07_CausalReasoning/mTOR_regulators_lp.rda"))
}else{
